...
- Local skills you can invoke live under `.claude/skills/` (e.g.
  `run-hakumai` for running the server/client when a human asks you to).
  Don't invoke build/run skills unless explicitly told to.
- `.claude/rules/` mirrors these pointers for Claude's rule-loading; the canon
  is still `.agents/`.
